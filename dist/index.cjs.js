...
var LabelsLayer = function LabelsLayer(_ref) {
  ...
  React.useEffect(function () {
    ...
    var updateImageOverlay = function updateImageOverlay() {
      var bounds = map.getBounds();
      var size = map.getSize(); // Project the bounds to EPSG:3005
      ...
      var projectedBounds = L__default["default"].bounds([sw.x, sw.y], [ne.x, ne.y]);

      if (imageOverlay) {
        imageOverlay.setUrl(url);
        imageOverlay.setBounds(projectedBounds);
      } else {
        imageOverlay = L__default["default"].Proj.imageOverlay(url, projectedBounds, {
          opacity: 1,
          interactive: false,
          crs: crs
        }).addTo(map);
...
var vectorTileStyling = {
  ...
  landcover: function landcover(properties, zoom) {
    ...
    if (grassClasses.includes(landcoverClass) && zoom >= 8) return [{
    ...
    if (landcoverClass === 'rock' && zoom >= 10) return [{
      color: legend.Landcover.rock,
      fill: true,
      fillOpacity: 1,
      weight: 0
    }];
    if (landcoverClass === 'sand' && zoom >= 10) return [{
