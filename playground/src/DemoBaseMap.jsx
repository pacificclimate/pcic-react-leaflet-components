...
function DemoBaseMap({
  ...
  const UpdateViewState = callbackOnMapEvents(["zoomend", "moveend"], (map) => {
  ...
  return (
    <React.Fragment>
      <Row>
        <Col xs={12}>
          <p>
            {numMaps} synchronized basemaps | View: {JSON.stringify(view)}
          </p>
        </Col>
      </Row>
      <Row>
        {map((i) => (
          <Col key={i} xs={colWidth}>
            ...
            >
              ...
              <UpdateViewState />
              <EEZLayer url="/tiles/eez/eez_boundaries_canada_v12.geojson" />
              <LayerGroup>
                {markers.map(({ lng, lat, comment }, i) => (
                  <CircleMarker
                    key={i}
                    center={{ lng, lat }}
                    radius={5}
